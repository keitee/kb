...
/*
called 4 time when pressing ">> <TAB>"

>>
add     delete  help    remove
>> keitee@kit-ubuntu:~/git/kb/code-cxx/cli/build$
...
so returns whole items.

when "del<TAB>"

>> delgenerator: t: del, s: 0
generator: add delete
return: delete
generator: t: del, s: 1
ete

so returns only matched item.

that is it's get called until return nullptr to say there is no more matches.

*/

const std::vector<std::string> words{"help", "add", "remove", "delete"};

// version1 which use words dictionaly
...
char *completion_generator_1(const char *text, int state)
{
  static std::vector<std::string> matches{};
  static size_t match_index{0};
...
char **completer_1(const char *text, int start, int end)
{
  // Don't do filename completion even if our generator finds no matches.
  rl_attempted_completion_over = 1;

  // Note: returning nullptr here will make readline use the default filename
  // completer. However, disable it above by doing:
  // rl_attempted_completion_over = 1;

  return rl_completion_matches(text, completion_generator_1);
}

/*
The comments from 2.6.4 A Short Completion Example

Attempt to complete on the contents of TEXT.  START and END bound the region of
rl_line_buffer that contains the word to complete.  TEXT is the word to
complete.  We can use the entire contents of rl_line_buffer in case we want to
do some simple parsing.  Return the array of matches, or NULL if there aren't
any.
...
>> help is not<TAB>
>> 012345678
>> help is nottext: not, [8, 11]

that's "start" is start pos of word when TAB is pressed

*/

char **completer_2(const char *text, int start, int end)
{
  ...
  if (start == 0)
  {
    ...
    matches = rl_completion_matches(text, completion_generator_1);
  }
