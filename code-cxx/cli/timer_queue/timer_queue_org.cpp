...
#define AS_LOG_SYS_ERROR(err, fmt) AS_LOG_ERROR(fmt " (%d)", err)
...
void TimerQueue::stop()
{
  ...
  if (mThread.joinable())
  {
    ...
    if (TEMP_FAILURE_RETRY(write(mEventFd, &doesntMatter, sizeof(uint64_t))) !=
        sizeof(uint64_t))
    {
 ...
 */
struct timespec
TimerQueue::calcAbsTime(const struct timespec &base,
 ...
 */
int64_t TimerQueue::add(const std::chrono::milliseconds &timeout,
                        bool oneShot,
                        const std::function<bool()> &handler)
{
  ...
  std::multiset<TimerEntry, TimerEntryCompare>::iterator it =
    mTimersQueue.emplace(entry);
...
bool TimerQueue::remove(int64_t tag)
{
  ...
  if (mExecCallbacks)
  {
    ...
    if ((mInCallbackTag == tag) &&
        (std::this_thread::get_id() != mThread.get_id()))
    {
  ...
  while (true)
  {
    ...
    else if (ret > 0)
    {
      ...
      if (fds[0].revents != 0)
      {
        ...
        uint64_t ignore;
        if (TEMP_FAILURE_RETRY(read(mEventFd, &ignore, sizeof(uint64_t))) !=
            sizeof(uint64_t))
        {
      ...
      if (fds[1].revents != 0)
      {
        ...
        uint64_t ignore;
        if (TEMP_FAILURE_RETRY(read(mTimerFd, &ignore, sizeof(uint64_t))) !=
            sizeof(uint64_t))
        {
