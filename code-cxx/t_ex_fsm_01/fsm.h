...
#define SYSTEM_STATUS_IS_OK(status) \
            (((uint32_t)(status)) == 1U)
...
typedef struct _fsm_init {
  ...
  /* used by tracing */
  void			  *data;					 /* any user data, that is sent to the 
                                  user after actions and timeouts */
  uint16_t		  init_state;				 /*initial state to be set for the machine */

  VRM_FSM_TRANS_CB  callback;					 /* called on transition */
  // FSM_TIMEOUT_CB	  timeout_callback;			 /* called on timeout */
