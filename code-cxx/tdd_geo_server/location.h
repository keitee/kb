...
public:
  ...
  double toCoordinate(double radians) const { return radians * (180 / Pi); }

  double latitudeAsRadians() const { return toRadians(latitude_); }

  double longitudeAsRadians() const { return toRadians(longitude_); }
  ...
  bool operator!=(const Location &that) { return !(*this == that); }
