...
RETURN
-------------------------------------------------------------------------------
sdevent/sdbus return scheme which is different from normal errno:
...
sd_event_default() or ad_event_new()?
...
All threads have exactly either zero or one default event loop objects
...
object is specified in the *event* parameter, the event source object is
returned in the *source* parameter. 
...
The example shows how to write a minimal UDP/IP server, that listens on port
7777. Whenever a datagram is received it outputs its contents to STDOUT, unless
it is precisely the string EXIT\n in which case the service exits. The service
will react to SIGTERM and SIGINT and do a clean exit then. It also notifies the
service manager about its completed startup, if it runs under a service manager.
Finally, it sends watchdog keep-alive messages to the service manager if it
asked for that, and if it runs under a service manager.
