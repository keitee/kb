...
static const char *ins_err =
  "ER_FSM_103: Instances still alive - can't destroy definition";
static const char *act_err =
  "ER_FSM_104: VRM_FSM_Input was called while executing action function";
static const char *act2_err =
  "ER_FSM_105: VRM_FSM_SetNextInput was called not from action function";
                         ...
                         VRM_FSM_DEFINITION_HANDLE *pHandle)
{
  ...
  if (SYSTEM_STATUS_IS_OK(stat))
  {
    ...
    for (i = 0u; i < pFSM_definition_info->num_entries; i++)
    {
          ...
          (pFSM_definition_info->entries[i].state != VRM_FSM_ALL_STATES))
      {
        ...
        if (pFSM_definition_info->entries[i].new_state <
            fsm_definition->min_state)
        {
          fsm_definition->min_state =
            pFSM_definition_info->entries[i].new_state;
        }
        if (pFSM_definition_info->entries[i].new_state >
            fsm_definition->max_state)
        {
          fsm_definition->max_state =
            pFSM_definition_info->entries[i].new_state;
        }
    ...
    fsm_definition->num_states =
      (fsm_definition->max_state - fsm_definition->min_state) + 1u;
    fsm_definition->num_inputs =
      (fsm_definition->max_input - fsm_definition->min_input) + 1u;
    count = fsm_definition->num_states * fsm_definition->num_inputs;

    std::cout << "fsm_definition::min_states: " << fsm_definition->min_state
              << std::endl;
    std::cout << "fsm_definition::max_states: " << fsm_definition->max_state
              << std::endl;
    std::cout << "fsm_definition::num_inputs: " << fsm_definition->num_inputs
              << std::endl;
    std::cout << "fsm_definition::num_states: " << fsm_definition->num_states
              << std::endl;
  ...
  if (SYSTEM_STATUS_IS_OK(stat))
  {
    ...
    for (i = 0u; i < pFSM_definition_info->num_entries; i++)
    {
      ...
      {
        offset =
          (fsm_definition->num_inputs * (state - fsm_definition->min_state)) +
...
VRM_FSM_DestroyDefinition(VRM_FSM_DEFINITION_HANDLE fsm_def_handle)
{
  ...
  if (SYSTEM_STATUS_IS_OK(stat))
  {
    ...
    if (def->instances_count > 0u)
    {
      DIAG_LOG_ERROR(
        VRM_diag_segment_id,
        ("Cannot destroy Definition, (%d) instances are still attached",
         def->instances_count));
      _fsm_error(ins_err);
      stat = VRM_FSM_FAILURE;
    }
...
SYSTEM_STATUS
VRM_FSM_CreateInstance(const VRM_FSM_INIT *pInit,
                       VRM_FSM_INSTANCE_HANDLE *pHandle)
{
  ...
  // sanity checking
  if ((pInit == NULL) ||
      (pInit->fsm_definition_handle == VRM_FSM_ILLEGAL_DEFINITION))
  {
    _fsm_error(par_err);
    stat = VRM_FSM_FAILURE;
  }
  ...
  if (SYSTEM_STATUS_IS_OK(stat))
  {
    ...
    if (!SYSTEM_STATUS_IS_OK(stat))
    {
      ...
      *pHandle = NULL;
      ...
      _fsm_error("Error encountered while trying to set initial state to new "
                 "FSM instance");
    }
    else
    {
  ...
  if (SYSTEM_STATUS_IS_OK(stat))
  {
    if ((input > def->max_input) || (input < def->min_input))
    {
      ...
      DIAG_LOG_ERROR(VRM_diag_segment_id,
                     ("Severe error encountered. fsm->name=%s, input=%d, "
                      "def->max_input=%d, def->min_input=%d",
                      fsm->name,
                      input,
                      def->max_input,
                      def->min_input));
...
SYSTEM_STATUS VRM_FSM_DestroyInstance(VRM_FSM_INSTANCE_HANDLE fsm_h)
{
  ...
  if (SYSTEM_STATUS_IS_OK(stat))
  {
    ...
    {
      DIAG_LOG_ERROR(VRM_diag_segment_id,
                     ("Something bad has happened. FSM instances count is zero "
                      "(0) when trying to destroy an instance"));
